...
def read_lines(src_translations_path: str, trg_translations_path: str) -> List[Tuple[str, str]]:
    """
...
def parse_nmt_output(nmt_output: Tuple[str, torch.Tensor]) -> Tuple[str, torch.Tensor]:
    """
    Takes the nmt_model output for a sentence, decodes the prediction, and returns the decoded string along with probablility score for each word 
    """
...
def generate_nmt_output(src_translations_path: str, trg_translations_path: str, output_save_path: str) -> None:
    """
    Parses the paralell corpora, runs the pretrained nmt model on the source sentences, 
    and saves the output results (probability of each word and text translation) as well as the target gold translation to a pickle file
    """
...
if __name__ == "__main__":
    generate_nmt_output("/Users/paigefink/human-assisted-nmt/nmt/corpus/spm/kyoto-train.ja", "/Users/paigefink/human-assisted-nmt/nmt/corpus/kftt-data-1.0/data/tok/kyoto-train.en", "test_nmt_out.p")